...
function AdminEquipos() {
  ...
  useEffect(() => {
    ...
    const ubicacionesChartData = Object.entries(equiposPorUbicacion).map(([ubicacion, cantidad]) => [ubicacion, cantidad]);

    // Establecer los datos para la gráfica de ubicaciones
    setUbicacionesData([['Ubicación', 'Cantidad de Equipos'], ...ubicacionesChartData]);

    // Contar equipos con y sin suscripción a Office
    const equiposConSuscripcion = equipos.filter(equipo => equipo.suscripcion_office === 'si').length;
    ...
    // Convertir los datos en el formato requerido por Google Charts
    const tiposEquiposChartData = Object.entries(equiposPorTipo).map(([tipo, cantidad]) => [tipo, cantidad]);

    // Establecer los datos para la gráfica de tipos de equipos
    setTiposEquiposData([['Tipo de Equipo', 'Cantidad de Equipos'], ...tiposEquiposChartData]);
  }, [equipos]);
